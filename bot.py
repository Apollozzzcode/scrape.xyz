"""
MERCURY — Discord Bot
Monitors Pasteview for hotmail pastes every 1 minute and posts URLs to a channel.
...
# ─── CONFIG ──────────────────────────────────────────────────────────────────
DISCORD_TOKEN  = "MTQ3NDk0NTY0ODk3MjUyOTkwNQ.GU5gnx.rezPtIBhQdIckN605qGGG0UaaIqyfpiOgymeDM"
CHANNEL_ID     = 1474944133285281915  # paste your channel ID here

CHECK_INTERVAL = 1        # minutes between auto-checks
PAGES_TO_SCAN  = 5        # archive pages per check
...
async def scrape_pasteview(num_pages: int = PAGES_TO_SCAN) -> list[dict]:
    """Returns list of {title, url} for all hotmail pastes found."""
    found = []
                    ...
                    for btn in buttons:
                        ...
                        if text and text.strip().lower() in ["next", ">", "»", "→", "▶"]:
                            ...
                            if disabled is not None or aria_dis == "true":
                                log.info(f"Last page reached at {page_num - 1}")
                                break
                            await btn.click()
...
async def post_pastes(channel: discord.TextChannel, pastes: list[dict]):
    if not pastes:
        return
    try:
        content = "\n".join(item["url"] for item in pastes)
        filename = f"hotmail_{datetime.now(timezone.utc).strftime('%Y%m%d_%H%M%S')}.txt"
        file = discord.File(fp=__import__('io').BytesIO(content.encode()), filename=filename)
...
if __name__ == "__main__":
    if DISCORD_TOKEN == "YOUR_BOT_TOKEN_HERE" or CHANNEL_ID == 123456789012345678:
        print("ERROR: Fill in your DISCORD_TOKEN and CHANNEL_ID in the CONFIG section.")
        raise SystemExit(1)
